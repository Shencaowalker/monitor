...
func DingdingrobotSend(config *viper.Viper) func(writer http.ResponseWriter, request *http.Request) {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		// 	log.Fatal(err)
		_= methods.Sendmessage(config, alarmdata)
		
	}
}

func DingdingrobotSendtest(config *viper.Viper) func(writer http.ResponseWriter, request *http.Request) {
	return func(writer http.ResponseWriter, request *http.Request) {
		// result := methods.Sendmessage(config, alarmdata)
		// if err := json.NewEncoder(writer).Encode(result); err != nil {
		// 	log.Fatal(err)
		methods.Sendmessagetest(config, "alert this is test")
	}
}
